...
export const mainController = {
  getIndex: async (request, response) => {
    ...
    try {
      ...
      if (user.errors) {
        //if any error responses come back, but technically the fetch is successful - see frontend/src/api/sample.js
        response.send({ error: "User not found or suspended." });
      } else if (user.data) {
        const userID = user.data.id; //proceeds to next phase
